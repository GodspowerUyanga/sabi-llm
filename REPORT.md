...
| Model invents a nonexistent tool call after finishing a task | Low, safe-by-construction | Re-tested 2026-08-25 (`demo/04`, `demo/05`): after correctly completing write→run→list, the model calls one extra tool it invents itself (`delete_file`/`delete_dir`) that isn't in the registry. `AgentLoop` rejects unknown tool names outright — the call fails with zero filesystem effect (verified: file/directory survive). Documents the tool allowlist working as a safety net; a tighter fix (stop once all requested steps report done) is a candidate follow-up, not required for correctness or safety. |
